...
class Stream(object):
    ...
    def __new__(cls):
        ...
        Stream._process = Process(target=cls._camera_loop, args=(cls.sensor_type, cls.frame, cls.status, cls._lock))
    ...
    def get_frame(self):
        ...
        with self._lock:
            array = numpy.frombuffer(self.frame.buf, dtype=numpy.uint8)
            array.shape = (CAM_HEIGHT, CAM_WIDTH, CAM_PIXELS)
    ...
    @classmethod
    def _camera_loop(cls, sensor_type: SensorType, frame: shared_memory.SharedMemory, status: shared_memory.SharedMemory, lock):
        ...
        while True:
            ...
            blurred_capture = cv2.GaussianBlur(raw_capture, (7, 7), 0)

            # Apply color thresholding for green
            hsv = cv2.cvtColor(blurred_capture, cv2.COLOR_RGB2HSV)
